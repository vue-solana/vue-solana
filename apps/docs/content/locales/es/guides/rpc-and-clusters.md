...
Nuxt guarda las opciones del módulo en runtime config público, así que las opciones deben ser serializables como JSON. El módulo Nuxt omite intencionalmente `payer` y `payerSecretKey`; nunca pongas un secreto crudo en la configuración runtime pública. Usa un plugin solo de cliente para un signer efímero, o usa un mensaje con un signer embebido de la wallet conectada.
