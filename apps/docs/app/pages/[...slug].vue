    ...
    <aside class="lg:sticky lg:top-0 lg:max-h-full lg:self-start lg:overflow-y-auto" aria-label="Documentation navigation">
      ...
      >
        <div class="mb-5 flex items-center justify-between gap-3">
          <div>
            <p class="text-xs font-bold uppercase tracking-[0.2em] text-violet-700 dark:text-violet-300">
              Docs
            </p>
        ...
        <nav class="grid gap-5">
          <section v-for="section in sections" :key="section.title" class="grid gap-2">
            <strong class="text-xs font-bold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">
              {{ section.title }}
            </strong>
