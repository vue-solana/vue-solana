...
- Solana primitives and transaction types: provided through `@vue-solana/core/web3`.
...
After a wallet is selected and connected, create a normal Solana transaction and send it with `useSignAndSendTransaction()`. Browser apps that create or serialize transactions should initialize the core Buffer polyfill before transaction code runs.
